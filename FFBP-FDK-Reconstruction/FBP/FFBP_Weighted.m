...
Log = Log(:,end:-1:1);
out_x = OutputSize;
out_y = size(Log,2); % 列数，对应投影角度数
[in_x,in_y] = size(Log); % in_x=探测器通道数，in_y=投影角度数
...
[Npix,Nproj] = size(Log);
% 探测器单个通道的角度间隔（弧度）：扇束总角度 / 原始采样数
Fan_sensor_spacing = (deg2rad(Fan_angle)/Norg);
% 投影角度增量（弧度）：总扫描角度 / 投影角度数
Bp_RotationIncrement = ((deg2rad(total_angle))/Nproj);
% 投影角度范围（弧度）：从起始角度到「起始+总角度」，线性均分
thetaI = deg2rad(0 + start_angle);
thetaF = deg2rad(total_angle + start_angle);
Bp_angles = linspace(thetaI, thetaF, Nproj);
% Parker加权专用角度数组（从0开始，无起始角度偏移）
Bp_park_ang = linspace(0, deg2rad(total_angle), Nproj);
% 投影角度实际间隔（验证增量，取相邻角度差的绝对值
Bp_spacing = abs(Bp_angles(1) - Bp_angles(2));
% 探测器原始通道数 & 每个通道对应的角度（居中分布）
Det_size = Norg;
Det_angles = Fan_sensor_spacing*linspace(-Det_size/2, Det_size/2, Norg)';
% 探测器通道的角度步长（相邻通道的角度差）
dif = diff(Det_angles(1:2));

% 补零后探测器通道数从 Norg 变为 Npix，需要扩展 Det_angles 以匹配尺寸
...
if strcmp(weighting,'differential')
    ...
    for i = 1:Nproj
      for j = 1:Npix
        if Bp_angles(i) >= 0 && Bp_angles(i) < 2*(deg2rad(Fan_angle/2) + Det_angles(j))
            mat(j,i) = Bp_angles(i)/(2*(deg2rad(Fan_angle/2) + Det_angles(j)));
        elseif Bp_angles(i) >= (2*(Det_angles(j) + deg2rad(Fan_angle/2))) && Bp_angles(i) < (pi + 2*Det_angles(j))
            mat(j,i) = 1;
        elseif Bp_angles(i) >= (pi + 2*Det_angles(j)) && Bp_angles(i) <= (pi+2*deg2rad(Fan_angle/2))
            mat(j,i) = (pi + 2*deg2rad(Fan_angle/2) - Bp_angles(i))/(2*(deg2rad(Fan_angle/2) - Det_angles(j)));
        else
            mat(j,i) = 0;
        end
      end
...
elseif strcmp(weighting,'parker')
    ...
    for i = 1:Nproj
      for j = 1:Npix
        if Bp_park_ang(i) >= 0 && Bp_park_ang(i) <= 2*(deg2rad(Fan_angle/2) - Det_angles(j))
            mat(j,i) = (sin((pi/4)*(Bp_park_ang(i)/((deg2rad(Fan_angle/2) - Det_angles(j))))))^2;
        elseif Bp_park_ang(i) >= 2*(deg2rad(Fan_angle/2) - Det_angles(j)) && Bp_park_ang(i) <= (pi - 2*Det_angles(j))
            mat(j,i) = 1;
        elseif Bp_park_ang(i) >= (pi - 2*Det_angles(j)) && Bp_park_ang(i) <= (pi + 2*deg2rad(Fan_angle/2))
            mat(j,i) = (sin((pi/4)*((pi + 2*deg2rad(Fan_angle/2) - Bp_park_ang(i))/((deg2rad(Fan_angle/2) + Det_angles(j))))))^2;
        else
            mat(j,i) = 0;
        end
      end
...
srad = sin(deg2rad(Fan_angle/2))*SOD;
