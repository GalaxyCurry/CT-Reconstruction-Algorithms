...
if parker
	proj = permute(ParkerWeight(permute(proj,[2 1 3]),geo,angles,parker),[2 1 3]); %%（u，v）（v，u）（u，v）
	diff_angles = diff(angles(1,:)); angle_step = mean(abs(diff_angles)); % to be used later
end 

filt_len = max(64,2^nextpow2(2*geo.nDetector(1)));
[ramp_kernel] = ramp_flat(filt_len); % Important Part
...
proj=permute(proj,[2 1 3]); %% （u，v）（v，u）
end
