function [res]=FDK_TIGRE(proj,angles,geo,varargin)
...
[filter,parker,dowang]=parse_inputs(angles,varargin);
...
for ii=1:size(angles,2)
    ...
    us = ((-geo.nDetector(1,1)/2+0.5):1:(geo.nDetector(1,1)/2-0.5))*geo.dDetector(1,1) + offset(1,ii);
    vs = ((-geo.nDetector(2,1)/2+0.5):1:(geo.nDetector(2,1)/2-0.5))*geo.dDetector(2,1) + offset(2,ii);
    [uu,vv] = meshgrid(us,vs); % detector
    
    % Create weight according to each detector element
    w = (geo.DSD(ii))./sqrt((geo.DSD(ii))^2+uu.^2 + vv.^2);
    
    % Multiply the weights with projection data（u，v）
    proj(:,:,ii) = proj(:,:,ii).*w';
end
...
offset = geo.offDetector(1,1);
offset = offset + (geo.DSD(1) / geo.DSO(1)) * geo.COR(1);

padwidth = fix(2*offset./geo.dDetector(1,1));
zgeo.offDetector(1,:) = geo.offDetector(1,:) - padwidth/2 * geo.dDetector(1,1);
zgeo.nDetector(1,1) = abs(padwidth) + geo.nDetector(1,1);
zgeo.sDetector(1,1) = zgeo.nDetector(1,1) * zgeo.dDetector(1,1);
...
%%
function [filter, parker, wang]=parse_inputs(angles,argin)
...
if mod(nVarargs,2)
    error('FDK:InvalidInput','Invalid number of inputs')
end
    ...
    switch opt
        ...
        case 'filter'
            if default
                ...
                if  ~ischar(val)
                    error('FDK:InvalidInput','Invalid filter')
        ...
        otherwise
            error('FDK:InvalidInput',['Invalid input name:', num2str(opt),'\n No such option in FDK()']);
    end
end
end
